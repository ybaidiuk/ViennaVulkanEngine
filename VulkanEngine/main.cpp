	...
	public:
		...
		*/
		...
		void createLights(uint32_t n, VESceneNode *parent) {
			...
			for (uint32_t i = 0; i < n; i++) {
				VELight *pLight;
				VECHECKPOINTER(pLight = getSceneManagerPointer()->createLight("Light" + std::to_string(i), VELight::VE_LIGHT_TYPE_POINT, parent));
		...
		virtual void loadLevel( uint32_t numLevel=1) {
            ...
            VESceneNode *e1, *e2, *e3;
			VECHECKPOINTER( e1 = getSceneManagerPointer()->loadModel("The Cube", "media/models/test/crate0", "cube.obj"));
            pScene->addChild(e1);
            e1->setTransform(glm::translate(glm::mat4(1.0f), glm::vec3(0.0, 5.0, 10.0)));
            createCubes(10, getRoot());
